...
export function Home() {
  ...
  const quickActions = [
    { name: t('user.create'), path: '/system/user', icon: Users },
    { name: t('department.title'), path: '/system/department', icon: Building2 },
    { name: t('role.title'), path: '/system/role', icon: UserCog },
    { name: t('permission.title'), path: '/system/permission', icon: ShieldCheck },
  ]
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg bg-gradient-to-r from-accent to-accent/80 p-8 text-white shadow-lg">
        <h1 className="text-3xl font-bold">
          {t('home.welcome', { name: user?.realName || (t('header.guest') as string) })}
        </h1>
            ...
            <Card key={stat.name} className="transition-shadow hover:shadow-lg">
              <CardContent className="p-6">
                ...
                <div className="mt-4 flex items-center text-sm">
                  ...
                  <span className="ml-1 text-muted-foreground">{t('home.stats.compare')}</span>
                </div>
              </CardContent>
            </Card>
          )
      ...
      <div className="rounded-lg bg-accent/10 p-6">
        <h3 className="mb-3 text-lg font-semibold text-accent">{t('home.quickStart')}</h3>
        <div className="space-y-2 text-sm text-foreground/80">
          <p>{t('home.startGuide.p1')}</p>
          <p>{t('home.startGuide.p2')}</p>
          <p>{t('home.startGuide.p3')}</p>
          <p>{t('home.startGuide.p4')}</p>
